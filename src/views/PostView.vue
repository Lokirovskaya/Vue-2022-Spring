<template>
  <ForumBorder>
    <el-breadcrumb separator-class="el-icon-arrow-right" style="margin: 20px; font-size: 15px;">
      ...
      <el-breadcrumb-item :to="{path:'/sector', query:{name:this.sector_name}}">{{sector_chinese_name[sector_name]}}
      </el-breadcrumb-item>
      <el-breadcrumb-item>{{this.posting_title}}</el-breadcrumb-item>
    </el-breadcrumb>
    <PostHead :posting_title="posting_title" :posting_id="posting_id" :posting_time="posting_time" :user_id="user_id"
      :username="username" :user_level="user_level" :user_photo="user_photo" :content="content" :like_count="like_count"
      :authority="authority" :resource="resource" :has_file="has_file" :like="like" @Like="ClickLike" @DelPost="DelPost"
      @ToComment="toComment"></PostHead>
    <br>
    <div v-for="item in cur_replys" :key="item.reply_id">
      <PostReply :reply_id="item.reply_id" :username="item.username" :user_level="item.user_level"
        :user_photo="item.user_photo" :content="item.content" :like_count="item.like_count"
        :reply_count="item.reply_count" :time="item.time" :like="item.like" :replys="item.replys"
        :posting_id="posting_id"></PostReply>
      <br>
    </div>
    <el-pagination layout="prev, pager, next,jumper" :total="reply_count" :page-size="20"
      @current-change="handleCurrentChange" style="margin-bottom: 15px">
    </el-pagination>
        ...
        this.$axios.post('/posting/getPostingInfo', qs.stringify({ posting_id: this.$route.query.id }), {
          ...
          .then(res => {
            if (res.data.errno === 0) {
              ...
              if (!this.$store.getters.is_admin && this.my_user_level < this.authority) {
