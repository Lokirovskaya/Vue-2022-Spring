<template>
  <el-container>
    ...
    <el-aside width="200px">
      ...
      <router-link :to="{path:'/personcenter', query:{user:username}}">
        <img :src="'http://43.138.55.69'+user_photo" class="qqtwo"
          style="height: 150px;width: 150px;position: relative;bottom: -6px">
    ...
    <el-container style="box-shadow: 0 2px 4px rgba(0, 0, 0, .12), 0 0 6px rgba(0, 0, 0, .04)">
      ...
      <el-footer height="25px">
        ...
        <el-tooltip class="item" effect="light" content="评论" placement="bottom">
          <img style="width: 20px;height: 20px;position: relative; left: 7px;bottom: -3px" alt="comment"
            src="../assets/comment.png" @click="see_Comment">
        </el-tooltip>
      ...
      <transition>
        <el-collapse-transition>
          <!--折叠动画效果-->
          <div v-show="SEE">
            <Reply_reply v-for="item in cur_replys" :reply_id="item.reply_id" :key="item.reply_id"
              :username="item.username" :content="item.content" :user_level="item.user_level"
              :user_photo="item.user_photo" :like_count="item.like_count" :time="item.time" :judge="item.judge"
              :like="item.like" :reply_to="item.reply_to" @Comment="Comment"></Reply_reply>
            <div style="background-color: #F6F6F6 ;">
              <el-pagination layout="prev, pager, next, jumper" :total="reply_count1" :page-size="5"
                @current-change="handleCurrentChange" style="margin-top: 10px" v-show="reply_count1 !== 0">
              </el-pagination>
              <div style="margin-left: 80px;margin-right: 80px">
                <el-input ref="COMMENT" type="textarea" style="margin-top: 10px;" placeholder="请输入内容" v-model="textarea"
                  maxlength="140" show-word-limit :disabled="this.$store.getters.is_banned">
                </el-input>
                <div style=" text-align: right ;margin-top: 10px">
                  <el-button size="mini" @click="f_Comment" style="position: relative;bottom: 5px;left: -5px">发表
                  </el-button>
                </div>
              </div>
